https://editor.p5js.org/cyprian.dove/sketches/tnnkM5g3y
function setup() {
  ...
  seedY = -100;
  seedWidth = 10;
  seedFall = random(10, 50);
  seedIsFalling = false;
  seedCaught = false;
  isFinchPressed = false;
  // Set up the canvas.
...
function draw() {
  ...
  let spritesInteract = (3600 <=  seedY && seedY >= 3600 - finchHeight);
let seedToLeft = (seedX < finchX);
   let seedToRight = (finchX + finchWidth < seedX)
      //distanceToEat = dist(finchX,finchY, seedX, seedY);
     if (spritesInteract && !seedToLeft && !seedToRight) {
      seedCaught=true;
       if (seedCaught==true);{
